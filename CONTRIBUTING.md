...
## 1. Modèle de branches (trunk-based)

Une seule branche longue durée :

| Branche | Rôle | Protection |
|---|---|---|
| `main` | **Tronc unique**. Reflète l'état de référence du projet (ce qui est buildable / soumis à TestFlight & App Store). | Aucun commit direct. Merge uniquement via PR validée. |

Toute autre branche est éphémère : créée pour un chantier, mergée dans `main` puis supprimée. Pas de branche `develop` ni de branches longue durée parallèles.
...
| `hotfix/` | Correctif **urgent** sur le tronc | `hotfix/auth-crash` |

Nommage : `prefix/kebab-case-court-et-descriptif`. Pas d'accents, pas d'espaces, pas de majuscules.

## 3. Workflow standard (toute contribution)

```
1. git checkout main
2. git pull origin main              # toujours à jour avant de brancher
3. git checkout -b feat/ma-feature   # nouvelle branche depuis main
4. # ... commits atomiques ...
5. git push -u origin feat/ma-feature
6. Ouvrir une PR  feat/ma-feature  →  main
7. Revue + validation utilisateur (CI verte : build + tests + checks)
8. Squash and merge dans main
9. Suppression de la branche distante
```

Un correctif urgent suit le même chemin avec le préfixe `hotfix/` et une revue accélérée.
...
docs: clarify branching model in CONTRIBUTING
```

## 5. Pull Requests

Chaque PR doit :
1. Cibler `main` (tronc unique).
...
- Stratégie de merge par défaut : **squash and merge** (historique linéaire sur `main`).
- Après merge, supprimer la branche distante.

## 7. Anti-patterns interdits

- ❌ Commits directs sur `main`
- ❌ Force push sur `main` ou une branche partagée
...
- ❌ Branches longue durée parallèles à `main`
